...
"""
#update: adapted from the sample code to see consistency
...
# tf.app.flags.DEFINE_string('data_dir', '/tmp/cifar10_eval',
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    images, labels = cifar10_inputs.inputs(data_dir=FLAGS.data_dir, batch_size=FLAGS.batch_size, isTraining=False, isRandom=False)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            num_iter = int(math.ceil(cifar10_inputs.NUM_EXAMPLES_PER_EPOCH_FOR_EVAL / FLAGS.batch_size))
            true_count = 0  # Counts the number of correct predictions.
            total_sample_count = num_iter * FLAGS.batch_size
            step = 0
            while step < num_iter and not coord.should_stop():
                predictions = sess.run([top_k_op])
                true_count += np.sum(predictions)
                print('{:s}: true_count @ 1 = {:d}'.format (datetime.now(), true_count))
