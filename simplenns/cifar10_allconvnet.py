# implements the paper:https://arxiv.org/pdf/1412.6806.pdf
...
def batch_norm_decorator(isTraining=True):
    ...
    def warpper(x):
        return tf.nn.relu(tcl.batch_norm(x, center=True, scale=True, is_training=isTraining, updates_collections=None))
    return warpper
...
def inference(images, isTraining=True):
    ...
    with tf.variable_scope('inference') as scope:
        conv11 = tcl.conv2d(images, 96, [3, 3], stride=1, activation_fn=batch_norm_decorator(isTraining=isTraining))
        conv12 = tcl.conv2d(conv11, 96, [3, 3], stride=1, activation_fn=batch_norm_decorator(isTraining=isTraining))
        # pool1 = tf.nn.max_pool(conv12, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
        #                        padding='SAME', name='pool1')
        pool1 = tcl.conv2d(conv12, 96, [3, 3], stride=2, activation_fn=tf.identity)

        conv21 = tcl.conv2d(pool1, 192, [3, 3], stride=1, activation_fn=batch_norm_decorator(isTraining=isTraining))
        conv22 = tcl.conv2d(conv21, 192, [3, 3], stride=1, activation_fn=batch_norm_decorator(isTraining=isTraining))
        # pool2 = tf.nn.max_pool(conv22, ksize=[1, 3, 3, 1],
        #                    strides=[1, 2, 2, 1], padding='SAME', name='pool2')
        pool2 = tcl.conv2d(conv22, 192, [3, 3], stride=2, activation_fn=tf.identity)

        conv3 = tcl.conv2d(pool2, 384, [8, 8], stride=1, padding='VALID', activation_fn=tf.nn.relu,variables_collections=['loss'])
